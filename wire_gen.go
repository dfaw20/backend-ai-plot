...
func initializeDIContainer(db *gorm.DB, oauth2Config2 oauth2.Config, config configuration.Config) dependency.DIContainer {
	...
	withdrawalExecuter := services.NewWithdrawalExecuter(db, userRepository, plotRepository, characterRepository, storyRepository)
	userHandler := handlers.NewUserHandler(oauth2Config2, userRepository, userTokenRepository, characterRepository, withdrawalExecuter)
